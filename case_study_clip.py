...
# Load CLIP model
model, _, preprocess = open_clip.create_model_and_transforms(EMBEDDING_MODEL, pretrained='openai')
...
def main():
    ...
    # if uploaded_file:
        ...
        embeddings = [encode_text(extracted_text) for extracted_text in st.session_state.extracted_texts]
        text_embeddings = np.vstack(embeddings)
        st.session_state.index = faiss.IndexFlatL2(text_embeddings.shape[1])
        st.session_state.index.add(text_embeddings)
        print(len(text_embeddings), len(st.session_state.image_paths), len(st.session_state.extracted_texts))
